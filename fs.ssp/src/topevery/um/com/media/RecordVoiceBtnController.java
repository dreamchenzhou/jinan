...
public class RecordVoiceBtnController extends Button
{
	...
	{ R.drawable.mic_1, R.drawable.mic_2, R.drawable.mic_3, R.drawable.mic_4, R.drawable.mic_5, R.drawable.cancel_record };
...
//				 手指上滑到超出限定后，显示松开取消发送提示
	...
	{
		// 存放录音文件目录
		String filePath = PathUtils.getRecordsPath();
		...
		// 录音文件的命名格式
		myRecAudioFile = new File(filePath, new DateFormat().format("yyyyMMdd_hhmmss", Calendar.getInstance(Locale.CHINA)) + ".amr");
		...
		mRecordHintTv.setText(mContext.getString(R.string.send_voice_hint));
		startRecording();
			...
			if (myRecAudioFile != null && myRecAudioFile.exists())
			{
				...
				if (mp != null)
				{
					...
					try
					{
						...
						{
							String name = String.format("%s", duration);
							ChatMessage chatMessage = new ChatMessage(myRecAudioFile, name);
							mAudioCallback.audioCallback(chatMessage);
						}
					}
					catch (Exception e)
					{
				...
				{
					Toast.makeText(mContext, mContext.getString(R.string.record_voice_permission_request), Toast.LENGTH_SHORT).show();
				}
			}
		}
	}
	...
	private void startRecording()
	{
		...
		{
			// HandleResponseCode.onHandle(mContext, RECORD_DENIED_STATUS,
			// false);
			cancelTimer();
			dismissDialog();
			if (mThread != null)
		...
		public void handleMessage(android.os.Message msg)
		{
			...
			if (controller != null)
			{
				...
				if (restTime > 0)
				{
					...
					// controller.mRecordHintTv.setText(String.format(controller.mContext.getString(R.string.rest_record_time_hint),
					// restTime));
					// 倒计时结束，发送语音, 重置状态
				}
				else if (restTime == 0)
				...
				else
				{
					...
					if (!controller.mTimeUp)
					{
						...
						// controller.mRecordHintTv.setText(controller.mContext.getString(R.string.move_to_cancel_hint));
						// else
						// controller.mRecordHintTv.setText(controller.mContext.getString(R.string.cancel_record_voice_hint));
						// 进入倒计时
					}
					else
					{
